...
class Curr_rate extends CI_Controller {
	private $c = 'curr_rate';
	...
	public function index($page=0)
	{
		...
		$config['total_rows'] = $this->CurrencyModel->jmlhdata_rate($perusahaan_id,$cabang_id);
		...
		$currency = $this->CurrencyModel->getlistrate($config['per_page'],$page,$perusahaan_id,$cabang_id);
		
		// prepare data
		$data['jmlh_data'] = $this->CurrencyModel->jmlhdata_rate($perusahaan_id,$cabang_id);
		$data['jmlh_item'] = $cfg[0]['jmlh_item'];
		$data['action'] = site_url().'/currency/edit';
		$data['results'] = $currency;
		$data['file'] = 'currency/list_rate';
	...
	public function edit($id=0){
		...
		$this->load->view('currency/form_currency_rate',$data);
