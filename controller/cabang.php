...
class Cabang extends CI_Controller {
	...
	public function index($page=0)
	{
		...
		if(!$this->acl->cek_acl($this->c,'index',$user_id)){
			$text = '<div class="notification warning no-margin"> <span class="strong">Warning!</span> You dont have authorizate to access CABANG page. </div><p></p>';
		...
		$config['total_rows'] = $this->CabangModel->jmlhdata($perusahaan_id);
		...
		$cabang = $this->CabangModel->getlist($config['per_page'],$page,$perusahaan_id);
		
		// prepare data
		$data['jmlh_data'] = $this->CabangModel->jmlhdata($perusahaan_id);
		$data['jmlh_item'] = $cfg[0]['jmlh_item'];
		$data['action'] = site_url().'/cabang/edit';
		$data['results'] = $cabang;
		$data['file'] = 'cabang/list';
